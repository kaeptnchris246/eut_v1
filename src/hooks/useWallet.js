...
import { formatEtherValue, getBrowserProvider, getJsonRpcProvider } from "@/lib/ethers-client.js";
...
function useWallet() {
  ...
  const fetchBalance = useCallback(async () => {
    ...
    try {
      ...
      if (context.provider) {
        ...
        const targetChain = context.chains.find((item) => item.id === context.chainId) ?? context.chains[0];
        if (targetChain?.rpcUrl) {
          provider = await getJsonRpcProvider(targetChain.rpcUrl);
        }
      }
      if (!provider) {
  ...
  }, [context.address, context.provider, context.chainId, context.chains]);
